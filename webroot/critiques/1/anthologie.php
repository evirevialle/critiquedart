<?php
		...
		if(isset($_GET['critique'])&&$_GET['critique']!=''){
			...
			$bio = file_get_contents('critiques/'.$url_auteur.'/bio.txt') or die("Le fichier de biographie bio.txt ne peut être chargé, il est soit absent soit corrompu.");
...
echo "<img src='images/pdf.jpg' height='20' width='20' title='pour afficher le pdf cliquer sur le lien, pour télécharger le pdf click droit et enregister sous' alt='logo PDF'></a></li>";
echo "<li>Sources d'archives identifiées <a href='critiques/".$url_auteur."/archives'>";
...
$fichier = fopen("critiques/".$url_auteur."/biblio.csv", "r") or die("Le fichier ne peut être chargé, il est soit absent soit corrompu.");
...
$fichier2 = fopen("critiques/".$url_auteur."/secondaire.csv", "r") or die("ne peut charger.");
...
$sitographie=array();
$fichier3 = fopen("critiques/".$url_auteur."/sitographie.csv", "r") or die("ne peut charger");
echo "<p><ul>";
