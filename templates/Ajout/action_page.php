<?php
	...
	if(!isset($_SESSION['user'])){
		echo '<script language="javascript">document.location.replace("/pages/home.php");</script>';
	}
	...
	echo $debut_header;
	if(isset($_SESSION['user'])){
		echo "<a href='/pages/home' class=\"icon-deconnect\" title=\"Se déconnecter\"></a></div>";
	} else {
		echo "<a href='/pages/unconnect' class=\"icon-person\" title=\"S’identifier\"></a></div>";
	}
	milieu_header();
	echo $fin_header;
?>
...
<?php
	...
	if(isset($_GET['auteurs']) AND $_GET['auteurs'] !=''){
		...
		print($this->Form->create(null, ['type' => 'post', 'url' => '/ajout/choisir_type_critique']));
		...
		print("</form>");
		print("<input type=\"button\" value=\"Annuler la saisie\" onclick=\"document.location.replace('/ajout/insertion');\" />");
	}
